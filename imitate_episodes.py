...
def eval_bc(config, ckpt_name, save_episode=True, num_rollouts=50):
    ...
    for rollout_id in range(num_rollouts):
        ...
        with torch.inference_mode():
            ...
            for t in range(max_timesteps):
                ...
                if config['policy_class'] == "ACT":
                    ...
                    if temporal_agg:
                        ...
                        exp_weights = torch.from_numpy(exp_weights).cpu().unsqueeze(dim=1)
                ...
                #         actuator_net_in = torch.from_numpy(actuator_net_in).float().unsqueeze(dim=0).cpu()
            ...
            if long_duration > 0:
                print(f'Warning: long durations:{long_duration}, > {DT} s, culmulated delay: {culmulated_delay:.3f} s')
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    for step in tqdm(range(num_steps+1)):
        ...
        if step % validate_every == 0:
            ...
            run.track(validation_summary, step=step, context=None)
        ...
        if (step > 0) and (step % eval_every == 0):
            ...
            run.track({'success': success}, step=step, context=None)
        ...
        run.track(forward_dict, step=step, context=None)
